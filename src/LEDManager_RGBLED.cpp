...
// the importance of this is to ensure the timer object is valid - some platforms are very limited
bool RGBLED::begin(void) {
   return _timer.begin();
}
...
void RGBLED::setState(const LEDState ledState, const uint32_t interval) {
   ...
   case LEDState::BLINK_ON:
      ...
      if ( ledState == LEDState::BLINK_ON ) {
         ...
         _timer.attachInterrupt(reinterpret_cast<CallbackArg>(&_RGBToggle), reinterpret_cast<void*>(this));
   ...
   case LEDState::ALTERNATE:
      ...
      _timer.attachInterrupt(reinterpret_cast<CallbackArg>(&_RGBAlternate), reinterpret_cast<void*>(this));
      _timer.setInterval(interval);
      _timer.arm(true);
      _timerArmed = true;
      break;
